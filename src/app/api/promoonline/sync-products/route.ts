...
export async function POST() {
  try {
    // 1. Call GraphQL to generate Excel
    ...
    // 2. Get Excel buffer from response (url, base64, or raw data)
    ...
    if (typeof excelResult === 'string') {
      ...
      // Try to find url or base64 field
      ...
      return NextResponse.json({ error: 'Unexpected ExcelResponse type', raw: excelResult }, { status: 422 })
    }
    ...
    if (!skuCol || !nameCol) {
      return NextResponse.json({
        error: 'Could not detect SKU or Name columns',
        headers,
      }, { status: 422 })
    }
